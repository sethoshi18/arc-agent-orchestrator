#!/usr/bin/env python3
"""
Deploy AgentOrchestrator to Arc Testnet using py-solc-x + web3.py.
...
def compile_contract(contract_path: Path):
    print("📋 Compiling AgentOrchestrator.sol...")
    install_solc("0.8.24")
    ...
    # "<stdin>:ContractName"
    contract_id = "<stdin>:AgentOrchestrator"
    if contract_id not in compiled:
        available = list(compiled.keys())
        print(f"ERROR: Could not find AgentOrchestrator in compiled output. Available: {available}")
        sys.exit(1)
...
def deploy(w3: Web3, abi, bytecode, deployer_account, cfg):
    print("🚀 Deploying AgentOrchestrator...")
    ...
    print(f"✅ AgentOrchestrator deployed: {deployed_address}")
...
def set_trusted_updater(w3: Web3, deployer_account, cfg, orchestrator_address: str):
    ...
    tx = registry.functions.setTrustedUpdater(
        Web3.to_checksum_address(orchestrator_address), True
    ).build_transaction({
        "from": deployer_account.address,
        "nonce": nonce,
        "maxFeePerGas": w3.to_wei(25, "gwei"),
...
def save_deployment(repo_root: Path, orchestrator_address: str, cfg: dict, receipt: dict):
...
def main():
    ...
    contract_path = repo_root / "contracts" / "AgentOrchestrator.sol"
    ...
    orchestrator_address, receipt = deploy(w3, abi, bytecode, deployer, cfg)

    set_trusted_updater(w3, deployer, cfg, orchestrator_address)

    save_deployment(repo_root, orchestrator_address, cfg, receipt)

    print("")
    print("🎉 Deployment complete!")
    print(f"   AgentOrchestrator : {orchestrator_address}")
    print(f"   View on ArcScan   : https://testnet.arcscan.app/address/{orchestrator_address}")
